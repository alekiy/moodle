 ...
 */
    ...
    $url = new moodle_url('/mod/richmedia/report.php');
	...
	if($action == "delete"){
		...
		$tab = explode(',',$joined);
		foreach ($tab as $elem){
			richmedia_delete_track($elem,$_POST['richmediaid']);
		}
		exit;
	}
    ...
    if (empty($download)) {
		...
		<script type="text/javascript">
			...
			var id = <?php echo $id?>;
			var richmediaid = <?php echo $richmedia->id?>;
			...
			function deleteAll(joined){
				var data= "action=delete&id="+id+"&richmediaid="+richmediaid;
				data += "&joined="+joined;
				http = createRequestObject();
				http.open('POST', 'report.php', false);
			...
			function deleteRows(){
				...
				if (checked.length == 0){
					alert('<?php echo get_string('noselectedline','richmedia') ?>');
				}
				else  {
	...
	if (empty($download)) {	
		...
		$table->finish_output();
		echo '<input type="button" value="'.get_string('selectall','richmedia').'" onclick="checkAll(true)" />';
		echo '/';
		echo '<input type="button" value="'.get_string('deselectall','richmedia').'" onclick="checkAll(false)" />';
		echo '<input type="button" value="'.get_string('deleteselection','richmedia').'" onclick="deleteRows()" />';
